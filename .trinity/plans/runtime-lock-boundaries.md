...
These snapshots may clone more than the absolute minimum at first. The important invariant is that cloning in-memory maps is bounded and predictable; disk and git I/O are not.
...
`SessionSnapshotBundle` should include the single session plus the repo-level attribution indexes needed by projections.
...
3. A synthetic slow worktree-status read does not hold the runtime mutex. Practical test shape:
   - snapshot a repo,
   - start a delayed status computation outside the lock,
   - concurrently call a pure runtime read or signal handler that needs the mutex,
   - assert it completes before the artificial delay.
4. `wait_for_work` behavior stays unchanged.
5. Homepage and session routes still render.

Do not add timing-sensitive tests that depend on machine speed. Use a controlled delayed future or injected status-reader trait if needed.
...
6. Add tests for multi-session list, single-session context, and the no-I/O-under-lock invariant.
7. Keep the public MCP / HTTP response shapes unchanged unless a separate plan explicitly shrinks them.

## Acceptance Criteria

- No route or MCP tool computes `plan_worktree_status` inside a `Runtime::read_repo` closure.
- `rg "compute_plan_worktree_status" src/server src/mcp_response.rs` shows no calls inside lock-held closures.
...
After this lands, the Leptos UI plan can build on the snapshot APIs and UI-specific response builders without carrying mutex-boundary work in its scope.
