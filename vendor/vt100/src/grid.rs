...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // clank vendor patch: save to scrollback whenever the scroll
            // region's top is row 0 (a line scrolled off the TOP of the
            // screen belongs in scrollback even with a bottom margin, e.g.
            // a TUI with a fixed bottom bar). Upstream used
            // `!self.scroll_region_active()`, which also required the
            // bottom margin to be default and so dropped those lines. This
            // matches xterm/zellij (scroll_region_top == 0). The alternate
            // grid has scrollback_len == 0, so alt-screen never accumulates.
            // See vendor/vt100/VENDOR.md.
    ...
    // clank vendor patch: no longer called (scroll_up now keys off
    // `scroll_top == 0`); kept verbatim so re-applying on an upstream bump
    // stays trivial. See vendor/vt100/VENDOR.md.
    #[allow(dead_code)]
