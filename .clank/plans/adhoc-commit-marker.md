...
- **Grouping (user's pick):** ad-hoc commits FOLD into the surrounding
  ...
  PRECEDING plan using EVENT ORDER, with the caller's direction passed
  EXPLICITLY (`newest_first`) — never inferred from author timestamps,
  which are second-resolution and can tie (codex daaa518 / 75a84c8).
  log / status / html pass newest-first. Only an ad-hoc older than every
  plan in the window is header-less. No separate "adhoc" header anywhere.
