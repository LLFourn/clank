//! Phase 2 contract tests for `clank-core`.
//!
//! Three categories:
//!
...
fn work_payload_write_feedback_round_trips() {
    let payload = WorkPayload {
        ...
        action: ExpectedAction::WriteFeedback {
            path: ".clank/feedback/foo/abc/codex.md".into(),
            target_sha: "abc".into(),
    ...
    assert_eq!(v["plan_id"], "clank/foo.md");
    assert_eq!(v["kind"], "write_feedback");
    assert_eq!(v["target_sha"], "abc");
    assert_eq!(v["path"], ".clank/feedback/foo/abc/codex.md");
    assert_eq!(v["plan_file"]["path"], ".clank/plans/foo.md");
...
fn ids_validate_on_deserialize() {
    ...
    let bad: Result<PlanId, _> = serde_json::from_value(json!("clank/.hidden.md"));
