...
fn wait_args_role(repo: &Path, author: &str, timeout: &str, role: Option<WaitRole>) -> WaitArgs {
    WaitArgs {
...
fn wait_args(repo: &Path, author: &str, timeout: &str) -> WaitArgs {
    wait_args_role(repo, author, timeout, Some(WaitRole::Reviewer))
}
...
async fn omitted_role_re_resolves_when_the_author_is_promoted_mid_wait() {
    ...
    let repo = env.repo();
    let err = clank::cli::wait::run(wait_args_role(repo, "rev", "2s", None))
        .await
        .expect_err("reviewer with no plans parks");
...
async fn explicit_role_survives_a_mid_wait_promotion() {
    ...
    // to master work; it stays parked to timeout.
    let env = parked_reviewer_env();
    let repo = env.repo();
    let waiter = tokio::spawn(clank::cli::wait::run(wait_args_role(
        repo,
    ...
    tokio::time::sleep(Duration::from_millis(1200)).await;
    promote_rev(repo);
    let err = tokio::time::timeout(Duration::from_secs(20), waiter)
        .await
        .expect("outer deadline")
        .expect("join")
        .expect_err("explicit reviewer role must not become master work");
    assert_timed_out(err);
}
...
async fn parked_wait_picks_up_a_tier_change_without_restarting() {
    ...
    // Sanity: under the CURRENT tiers the wait truly parks (times out).
    let err = clank::cli::wait::run(wait_args(repo, "rev", "2s"))
        .await
        .expect_err("non-member has no work → parks");
    assert!(
        err.downcast_ref::<clank::cli::wait::WaitTimeout>()
            .is_some(),
        "expected WaitTimeout, got: {err:#}"
    ...
    let waiter = tokio::spawn(clank::cli::wait::run(wait_args(repo, "rev", "30s")));
    tokio::time::sleep(Duration::from_millis(1200)).await;
