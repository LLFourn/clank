//! Phase 0 of `clank-core-unification.md`: schema baselines for
        ...
        panic!(
            "wire snapshot missing: {}\n\
             If this is a new wire shape, generate it with:\n  \
               UPDATE_SNAPSHOTS=1 cargo test -p clank-core --test wire_snapshots",
            path.display(),
        )
    });
    assert_eq!(
...
fn feedback_fixture() -> Feedback {
    Feedback {
        ...
        body: "APPROVE\n\nlgtm\n".into(),
        path: ".clank/feedback/foo/abc/alice.md".into(),
...
fn pr_hint_fixture() -> PrHint {
    PrHint {
        ...
        options: vec![PrHintOption {
            ...
            base: "main".into(),
            command: "git merge --squash clank/foo".into(),
        }],
...
fn plan_row_fixture() -> PlanRow {
    PlanRow {
        repo: "/r".into(),
        plan_id: Some("clank/foo.md".into()),
        slug: "foo".into(),
        lifecycle: PlanLifecycle::Active,
        current_path: ".clank/plans/foo.md".into(),
        ...
        gate_state: Some(clank_core::vocab::CommitGateState::Unreviewed),
...
fn schema_list_plans_response() {
    let v = serde_json::to_value(ListPlansResponse {
        ...
        conflicts: vec![PlanConflict {
            plan_id: Some("clank/foo.md".into()),
            slug: "foo".into(),
            paths: vec![".clank/plans/foo.md".into()],
        }],
    })
    .unwrap();
...
fn schema_diff_response() {
    let v = serde_json::to_value(DiffResponse {
        from: "abc".into(),
        to: "def".into(),
        from_path: ".clank/plans/foo.md".into(),
        to_path: ".clank/plans/foo.md".into(),
        diff_files: vec![file_diff_fixture()],
    })
    .unwrap();
...
fn schema_mcp_error_plan_not_active() {
    let v = serde_json::to_value(McpErrorPayload::PlanNotActive {
        plan_id: "clank/foo.md".into(),
        message: "plan clank/foo.md is finalized".into(),
    })
    .unwrap();
...
fn schema_start_plan_response() {
    let v = serde_json::to_value(StartPlanResponse {
        plan_id: "clank/foo.md".into(),
        repo: "/r".into(),
        canonical_path: "/r/.clank/plans/foo.md".into(),
        slug: "foo".into(),
        committed: false,
