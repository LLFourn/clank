...
///
/// Gate precedence: ChangesRequested > Finished > Approved >
/// Unreviewed (a single FINISHED vote pushes the gate to
/// Finished even alongside plain APPROVEs). Routing:
/// - ChangesRequested → MasterToRevise
/// - Finished + clean worktree → MasterToFinalize
/// - Approved + clean worktree → MasterToContinue (keep
///   working; FINISHED hasn't been granted yet)
/// - Either Finished or Approved with BodyDirty → MasterToCommit
/// - Unreviewed with missing participants → ReviewerApprovalsMissing
fn evaluate(
    feedback: &FeedbackView,
    ...
    let mut finishers: Vec<AgentLabel> = Vec::new();
    ...
    for p in &participants {
        match target.get(p).map(|e| e.verdict) {
            ...
            Some(Verdict::Finished) => finishers.push(p.clone()),
    ...
    } else if !finishers.is_empty() && missing.is_empty() {
        CommitGateState::Finished
    } else if (!approvers.is_empty() || !finishers.is_empty()) && missing.is_empty() {
        // finishers.is_empty() at this point — Finished branch
        // is exhausted, so approvers must be non-empty.
...
mod tests {
    ...
    #[test]
    fn approved_routes_to_master_to_continue_regardless_of_touched_code() {
        // APPROVE on a plan-only commit, on a code commit, or on
        // a mixed commit — all route to MasterToContinue. The
        // touched_code dimension no longer steers the gate;
        // FINISHED is the only thing that unlocks finalize.
        for (label_str, evt_args) in [
            ...
            let s = state_with_one_plan(
                &key,
                vec![evt("1111", 1, evt_args.0, evt_args.1)],
            );
            let mut entries = BTreeMap::new();
            ...
            assert!(
                matches!(view.waiting_on, WaitingOn::MasterToContinue),
                "{label_str}: expected MasterToContinue, got {:?}",
                view.waiting_on
            );
        }
    }
    ...
    #[test]
    fn finished_routes_to_master_to_finalize() {
        // A single FINISHED vote unlocks finalize, even on a
        // plan-only commit (the research path).
        let key = plan("foo");
        let s = state_with_one_plan(&key, vec![evt("1111", 1, true, false)]);
        let mut entries = BTreeMap::new();
        entries.insert(label("alice"), entry(Verdict::Finished));
        let fb = FeedbackView {
            per_commit: vec![CommitFeedback {
                sha: sha("1111"),
                entries,
        ...
        .unwrap();
        assert_eq!(view.gate_state, CommitGateState::Finished);
