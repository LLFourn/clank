//! Public response DTOs for Clank's HTTP / MCP / SSE surfaces.
//!
...
pub enum TimelineEvent {
    ...
    /// approvals are sealed in `.clank/finished/<stem>/` at this
    /// commit's tree.
...
/// Approval snapshot file under `.clank/finished/<stem>/<agent>.md`
...
/// `clank status --json` response body. Repo identity + a folded
...
pub struct StatusResponse {
    ...
    /// path that Clank uses as the repo identity).
...
pub struct DeleteRepoOutcome {
    ...
    /// `.clank-watched` registry file failed. The repo will
    /// reappear on daemon restart until the operator fixes the
    /// file.
    pub registry_write_error: Option<String>,
}

/// `finish_preview` response — everything `clank finish` needs to
...
/// (e.g. `clank finish --dry`) and must not be recombined into a
...
pub enum FinalizeReadiness {
    ...
    /// and commit.
    Ready,
    /// Plan is already finalized — `clank finish` exits 0 with
...
/// One specific reason `clank finish` cannot proceed.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "kind", rename_all = "snake_case")]
...
/// `rewrite_preview` response — the typed manifest `clank purge`
/// and `clank finish --squash`/`--purge` execute. Returned by
...
/// what counts as a plan-`.clank/` path) live in the daemon's
/// projection — the CLI walks `commits` in order and applies the
/// per-commit disposition without re-deriving Clank attribution.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
pub enum RewriteDisposition {
    /// Commit touched ONLY this plan's `.clank/` paths. The
    /// rewrite engine skips it entirely; the parent chain hops over.
    Drop,
    /// Commit didn't touch any of this plan's `.clank/` paths.
    /// Reuse the SHA verbatim in the rewritten chain.
    KeepVerbatim,
    /// Commit touched this plan's `.clank/` paths AND other
...
pub struct PurgeAllPreviewResponse {
    ...
    /// `.clank/` path. `None` when no commit ever touched
    /// `.clank/` — the CLI short-circuits with "nothing to purge"
    /// in that case.
    pub intro_sha: Option<crate::ids::CommitSha>,
...
pub enum McpErrorPayload {
    ...
    /// `plan_id` referred to a basename Clank isn't watching.
    UnknownRepo { basename: String },
    /// Two `.clank/plans/` files with the same stem; daemon
